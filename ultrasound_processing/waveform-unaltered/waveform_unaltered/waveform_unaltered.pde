...
void serialEvent(Serial port) {
  ...
  if (FPGA) {
    ...
    processSample(port.readStringUntil(10), true); //linefeed in ASCII
}
...
void keyPressed() {
  if (key == CODED) {
    ...
    case '[':
      ...
      if(sensorIndex == 0) {
        sensorIndex = SENSOR_COUNT - 1;
      } else {
        sensorIndex--;
      }
      displayMessage("Selected Sensor: " + sensorIndex);
      break;
    case ']':
      // Select next sensor
      if(sensorIndex == (SENSOR_COUNT - 1)) {
        sensorIndex = 0;
      } else {
        sensorIndex++;
      }
